...
class SMHIWeather:
        ...
        if location not in self.LOCATIONS:
            return {
                "error": (
                    ...
                    f"Available: {', '.join(self.LOCATIONS.keys())}"
                )
            }
            ...
            parsed.append(
                {
                    ...
                    "feels_like": params.get(
                        "t"
                    ),  # Simplified - SMHI doesn't provide feels_like
    ...
    def _get_highest_severity(self, warnings: List[Dict]) -> str:
        ...
        if not warnings:
            return "NONE"
        highest = max(
            warnings, key=lambda w: severity_order.get(w.get("severity", "NONE"), 0)
        )
    ...
    def get_warnings_summary(self, county: str = "Östergötland") -> str:
        ...
        sorted_warnings = sorted(
            warnings, key=lambda w: severity_order.get(w.get("severity", "YELLOW"), 99)
        )
